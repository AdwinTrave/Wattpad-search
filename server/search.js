...
Meteor.methods({
  ...
  identifyCategories: function(queryArray, stem){
      ...
      categories.some(function(cat)
      {
        ...
        else if(cat.terms.length > 0)
        {
          cat.terms.some(function(term)
          {
            ...
            //stem if this is the second round
            if(stem)
            {
              token = stopWordsAndStem(token);
            }
  ...
  stopWordsAndStemer: function(queryArray){
    return stopWordsAndStem(queryArray);
  },
...
function stopWordsAndStem(queryArray)
{
